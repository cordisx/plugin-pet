...
export function DeviceControl({ item, state, busy, run, navigation }: DeviceProps) {
  ...
  return <section className="pet-reservoir" aria-label={item.name}>
    <div className="pet-reservoir-heading"><PetDeviceArt device={item.device} tier={capacity.tier} fill={capacity.capacity ? stored/capacity.capacity : 0} foodId={state.devices.foodQueue[0]?.foodId} enabled={enabled} /><div><strong>{item.name}</strong><small>{stateLabel}</small></div><Button variant="ghost" aria-label={owned ? `${enabled ? '暂停' : '启用'}${item.name}` : `购买${item.name}`} title={owned ? enabled ? '暂停设备' : '启用设备' : '前往商店'} disabled={busy} onClick={() => owned ? run({ type: 'device-settings', value: water ? { waterEnabled: !enabled } : { feederEnabled: !enabled } }) : openDeviceProduct(navigation, item)}><DeviceIcon kind={owned ? 'power' : 'shop'} /></Button></div>
    {owned && <><div className="pet-reservoir-level"><span>{water ? '储水' : '储粮'} <strong>{Math.floor(stored)} / {capacity.capacity}</strong></span><small>每轮最多 {capacity.serviceLimit} 只 · 全体共用</small></div><meter aria-label={water ? '储水量' : '储粮量'} min={0} max={capacity.capacity} value={stored} />
      {water ? <div className="pet-reservoir-fill"><small>{stored <= 0 ? '加水后恢复供给' : '饮水低于 35 时自动补充'} · 免费清水</small><Button variant="ghost" title="补满储水箱" aria-label="加满清水" disabled={busy || free <= 0} onClick={() => run({ type: 'device-refill-water', quantity: free })}><DeviceIcon kind="fill" />加满</Button></div>
...
.pet-reservoir{display:flex;flex-direction:column;gap:10px;padding:14px;border:1px solid color-mix(in srgb,currentColor 12%,transparent);border-radius:16px;min-width:0}
.pet-reservoir-heading{display:flex;align-items:center;gap:12px}.pet-reservoir-heading>div{display:flex;flex-direction:column;gap:5px;flex:1;min-width:0}.pet-reservoir-heading>img{width:64px;height:64px}.pet-reservoir small{font-size:11px;line-height:1.4;opacity:.7}
