...
test('declares exact controlled render and optional pointer capabilities', () => {
  ...
  assert.deepEqual(inject, ['extensionPointVisuals', 'documents', 'pages', 'routes', 'slots', 'managerContent'])
})
test('activation registers lazy visuals after loading; hiding and disposal restore native seats', async () => {
  ...
  let update, value, revision = 0, disposed = 0, stopped = false
  apply({
  ...
  assert.ok(navigation.every(item => item.tabs.length === 5))
