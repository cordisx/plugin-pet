...
test('client exposes attribution/denied state and never rewards partial profile usage', async t => {
  ...
  await client.start()
  assert.equal(client.getSnapshot().usage.reason, 'usage-attribution-unavailable')
  snapshot = { ...snapshot, revision: 1, eligibleTokens: 10000, inputTokens: 10000 }
  await client.refreshUsage()
  assert.equal(client.getSnapshot().state.wallet.balance, 0)
